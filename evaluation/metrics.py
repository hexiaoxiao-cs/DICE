...
class MetricEvaluator:
    def __init__(self, device):
        ...
        # Load the pre-trained LPIPS model (VGG-based is the default)
        # self.loss_fn_vgg = lpips.LPIPS(net='vgg').to(device)
        # self.loss_fn_alex = lpips.LPIPS(net='alex').to(device) # Can also use a different network, e.g., AlexNet
        # # Load the pre-trained CLIP model and processor
        # model_name = "openai/clip-vit-base-patch32" 
        # self.model_CLIP = CLIPModel.from_pretrained(model_name).to(device)
        # self.processor_CLIP = CLIPProcessor.from_pretrained(model_name).to(device)
        ...
        self.model_siglip = AutoModel.from_pretrained("google/siglip-base-patch16-224").to(device)
        self.processor_siglip = AutoProcessor.from_pretrained("google/siglip-base-patch16-224")
        
        
        #Preload all models to GPU
        
    def calculate_metric(self, metric_name, img_pred, img_gt, **kwargs):
        """Dispatch to the appropriate metric calculation method.
        
        Args:
            metric_name: Name of the metric (e.g., 'psnr', 'lpips', 'mse', 'ssim', 
                         'structure_distance', 'clip_similarity').
            img_pred: Predicted/edited image (tensor or PIL Image).
            img_gt: Ground truth image (tensor or PIL Image), or text for CLIP similarity.
            **kwargs: Additional keyword arguments passed to the metric method.
        
        Returns:
            Computed metric value.
        """
        method = getattr(self, f"calculate_{metric_name}", None)
        if method is None:
            raise ValueError(f"Unknown metric: {metric_name}. Available: "
                             f"psnr, lpips, mse, ssim, structure_distance, clip_similarity")
        return method(img_pred, img_gt, **kwargs)
    ...
    def calculate_dino_similarity(self,image1,image2,model,processor):
        with torch.no_grad():
            inputs1 = processor(images=image1, return_tensors="pt").to(self.device)
        ...
        with torch.no_grad():
            inputs2 = processor(images=image2, return_tensors="pt").to(self.device)
    ...
    def calculate_SigLip_score(self,image1,text):
        # Preprocess the inputs
        inputs = self.processor_siglip(text=[text], images=image1, return_tensors="pt", padding=True)

        # If a GPU is available, move the model and inputs there
        with torch.no_grad():
            outputs = self.model_siglip(**inputs)
