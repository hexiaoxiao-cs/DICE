"""
DICE Inversion + Editing for LLaDA.

Applies DICE to LLaDA (Large Language Diffusion with mAsking), an 8B-parameter
discrete diffusion language model. Uses the mask token instead of random noise
for the forward noising process.
...
# Argument Parsing
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="DICE Text Editing with LLaDA")
...
parser.add_argument("--task_start", type=int, default=0,
                    help="Start index for dataset range")
parser.add_argument("--task_end", type=int, default=65,
                    help="End index for dataset range (relative to offset 700)")
parser.add_argument("--max_t_range", nargs=2, type=int, default=[70, 100],
                    help="max_t sweep range (percentage)")
parser.add_argument("--lamb_range", nargs=2, type=int, default=[20, 90],
                    help="Lambda sweep range (percentage)")
...
parser.add_argument("--output_dir", type=str, default="results_llada",
...
# Load Model & Dataset
# ---------------------------------------------------------------------------
print(f"Loading LLaDA-8B-Instruct on {device}...")
...
# ---------------------------------------------------------------------------
all_inputs = []
for i in range(700 + args.task_start, 700 + args.task_end):
    i = str(i)
    if i not in dataset:
        continue
...
for current_idx, inputs_1, only_text_2, inputs_forward in all_inputs:
    ...
    for max_t, lamb in pbar:
        ...
        with torch.inference_mode(), torch.autocast(device_type="cuda"):
            # Locate text_2 boundary (LLaDA uses token 220 as separator)
            loc_of_text_2_start = torch.where(inputs_1["input_ids"] == 220)[1][-1] + 1
            
            masks = generate_mask_schedule(
                timesteps, latent_shape=(len(only_text_2["input_ids"][0]),), max_t=max_t)
            
            masks_only = masks.copy()
            new_masks = []
            for mask in masks:
                new_mask = torch.zeros_like(inputs_1["input_ids"])
                new_mask[:, loc_of_text_2_start:] = mask
            ...
            # LLaDA uses mask token instead of random noise
            current_noise = torch.full([noise_size], MASK_TOKEN_ID).to(device)
            ...
            loc_of_text_2_start_new = torch.where(inputs["input_ids"] == 220)[1][-1] + 1
            ...
            for mask in masks_only:
                ...
                new_mask[:, loc_of_text_2_start_new:] = mask
                new_masks.append(new_mask)
            masks = new_masks
